...
# class SurrogateFusionModel(nn.Module):
#     def __init__(self, num_emotions, hidden_size=128):
#         """代理模型融合结构，用于替代原始门控网络"""
#         super(SurrogateFusionModel, self).__init__()
...
#         )
#
#     def forward(self, distill_logits, qwen_logits):
#         x = torch.cat([distill_logits, qwen_logits], dim=1)
#         return self.mlp(x)
...
class SurrogateFusionModel(nn.Module):
    ...
    def forward(self, distill_logits, qwen_logits, return_feature=False):
        x = torch.cat([distill_logits, qwen_logits], dim=1)

        hidden = self.mlp[0](x)     # 第一层 Linear
        hidden = self.mlp[1](hidden)  # ReLU
        out = self.mlp[2](hidden)   # 第二层 Linear
...
def create_gate_model(gate, num_labels, device):
    ...
    elif gate == 'SurrogateFusion':
        return SurrogateFusionModel(num_labels).to(device)
    else:
...
def load_distilled_model(qwen_model, dataname, num_labels, depth, device):
    ...
    state_dict = torch.load(distill_path, map_location="cpu")
...
def main():
    ...
    parser.add_argument("--gate", type=str, default='SurrogateFusion')
